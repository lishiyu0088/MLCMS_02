...
public class SIRGroupModel extends AbstractGroupModel<SIRGroup> {
	...
	/*Define two variable timer and previousTime to check the state per second*/
    ...
    @Override
	/*Timer is used to record the accumulative time. If the time is large than 1, then the state will be updated. The timer
	* will be set to 0 and previous time will also be updated.*/
    public void update(final double simTimeInSec) {
		/*Include the case of recovery state: choose from the infective group to be removed*/
        DynamicElementContainer<Pedestrian> c = topography.getPedestrianDynamicElements();
        LinkedCellsGrid<Pedestrian> neighbors;
        List<Pedestrian> n;

        if (this.timer < 1) {
            this.timer = this.timer + (simTimeInSec - previousTime);
        } else {
            if (c.getElements().size() > 0) {
        ...
        this.previousTime = simTimeInSec;
    }
}